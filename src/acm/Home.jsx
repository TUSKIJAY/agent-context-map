...
export function Home({ recent, onNew, onViewSample, onOpenRecent, onDeleteRecent, onImport, persistenceMode, accent = "#6366f1" }) {
      ...
      <div style={{ width: 760, maxWidth: "92vw", padding: "8vh 0 60px" }}>
        ...
        {recent.length === 0 ? (
          <div style={{ border: "1px dashed #dfe3ea", borderRadius: 12, padding: "30px 20px", textAlign: "center",
            ...
            还没有保存过的图谱。点「新建图谱」或「查看示例」开始，编辑会自动保存到本地。
          </div>
        ) : (
        ...
        {persistenceMode === "local" && (
          <div style={{ marginTop: 24, fontSize: 11, color: "#b3bac6", textAlign: "center" }}>
            浏览器预览模式：数据暂存于本地缓存。桌面版（exe）使用 SQLite 持久化到应用数据目录。
          </div>
        )}
      </div>
    </div>
