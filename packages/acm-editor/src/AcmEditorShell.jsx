...
import { LeftRail, Inspector, AgentPanel, SuggestionsPanel, DiffPanel, ValidatePanel, ghostBtn } from "./Panels.jsx";
import {
  useTweaks, TweaksPanel, TweakSection, TweakToggle, TweakRadio, TweakColor,
} from "./TweaksPanel.jsx";
import { Home } from "./Home.jsx";
import { assertEditorPlatform } from "./contracts.js";
import { useDocumentController } from "./document-controller.js";
...
export default function AcmEditorShell({ platform }) {
  const injected = useMemo(() => assertEditorPlatform(platform), [platform]);
  const { store, files, agent, exportAdapter, host, capabilities } = injected;
  ...
  const documentController = useDocumentController(BLANK_DOC);
  const { document: doc, baseline: base, historyRef, epochRef: docEpochRef } = documentController;
  ...
  const loadRecord = (rec, savedVp) => {
    ...
    documentController.load(rec.body, rec.base_snapshot ? rec.base_snapshot : clone(rec.body));
  ...
  const commit = documentController.commit;
  ...
  const undo = () => { if (documentController.undo()) clearViewLayout(); };
  const redo = () => { if (documentController.redo()) clearViewLayout(); };
  ...
  const runAgent = async (text) => {
    ...
    let patch = null;
    try {
      patch = await agent.requestAgentPatch({ doc, baseNodeId, prompt, selection });
    } catch (e) {
      setAgentBusy(false);
  ...
  const movePendingNode = (nodeId, x, y) => setPendingAgentPatch((p) => {
    ...
    return { ...p, operations: p.operations.map((op) => (
      op.status === "pending" && op.op === "addNode" && op.node?.id === nodeId
        ? { ...op, node: { ...op.node, x, y } }
        : op
    )) };
  });
  ...
  const onSave = async () => {
    ...
    documentController.adoptBaseline(clone(doc));
    try {
  ...
  const deleteRecent = async (id) => {
    if (store.persistenceMode === "project" && !host.confirm("这会删除项目中的正式 ACM-MD 文件，并在本机状态目录保留恢复副本。确定继续吗？")) return;
    try {
  ...
  const onImport = async () => {
    const f = await files.openTextFile();
    if (!f) return;
  ...
  const migrateLegacy = async () => {
    try {
      ...
      if (invalid.length) {
        ...
        host.alert(`迁移预览发现 ${invalid.length} 份非法文档；它们不会被自动修补或写入。\n\n${summary}`);
      }
  ...
  useFx(() => {
    ...
    return host.subscribeKeydown(h);
  });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", background: "#fff", color: "#1d2433" }}>
      ...
      {candidate && <CandidateMenu cand={candidate} onPick={pickCandidate} onClose={() => setCandidate(null)} nameOf={nameOf} doc={doc} host={host} />}
      {exportTab && <ExportModal doc={doc} base={base} diff={diff} tab={exportTab} setTab={setExportTab} onClose={() => setExportTab(null)} showToast={showToast} files={files} />}
  ...
  return (
    <div style={{ width: 356, flex: "0 0 356px", height: "100%", borderLeft: "1px solid #ebedf1", display: "flex", flexDirection: "column", background: "#fff", minHeight: 0 }}>
      <div style={{ display: "flex", borderBottom: "1px solid #ebedf1", flex: "0 0 auto" }}>
        ...
        <Tab id="suggestions" label="待采纳" badge={pendingAgentStats?.total} badgeColor="#8b5cf6" />
        <Tab id="diff" label="正式 Diff" badge={diffN} badgeColor="#2563eb" />
...
function CandidateMenu({ cand, onPick, onClose, nameOf, doc, host }) {
  const fromN = doc.nodes.find((n) => n.id === cand.from), toN = doc.nodes.find((n) => n.id === cand.to);
  const viewport = host.getViewportSize();
  const x = Math.min(cand.pos.x, viewport.width - 250), y = Math.min(cand.pos.y, viewport.height - 60 - cand.options.length * 40);
  return (
    <>
...
function ExportModal({ doc, base, diff, tab, setTab, onClose, showToast, files }) {
  ...
  const saveToFile = async () => {
    try { const r = await files.saveTextFile(content, { defaultName: fileName }); if (r) showToast("已保存到 " + r.path); }
    catch (e) { showToast("保存失败：" + (e?.message || e)); }
  };
